...
Changes in 1.4.1
  • Anchor Y-positions raised to +0.75 m (strings start near top of window)
  • Initial string lengths are computed from anchor→joint distance, so pose
    is unchanged.
  • Retains per-string velocity damping (v1.4.0) to quell oscillations.
"""
...
ANCHOR_Y            = 0.75           # ↑ anchors moved toward top
LEFT_WAVE_A, LEFT_WAVE_F = 0.05, 2.0 # m, Hz
STRING_DAMP_GAIN    = 0.02           # m / (m·s)  (energy bleed)
...
# strings (SlideJoint)  – initial length = current taut distance
strings, cmd_len = {}, {}
def add_string(k):
...
for name, pj in limbs.items():
    slider(f"limb {name}", pj.distance,(pj.distance*0.3, pj.distance*2))
y += 8
...
def damp_strings(dt):
    ...
    for k,sj in strings.items():
        ...
        cmd_len[k]  = max(0.01, cmd_len[k])       # avoid zero/negative
...
while running:
    ...
    for lbl,s in ui.items():
        ...
        if lbl.startswith("len "):
            ...
            limbs[lbl[5:]].distance = val
        elif lbl.startswith("X "):
            k = lbl[2:]; A[k] = pymunk.Vec2d(val, A[k].y); strings[k].anchor_a = A[k]
